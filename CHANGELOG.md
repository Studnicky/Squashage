...
- **Native record/draft fan-out.** Per-record and per-draft processing use the engine's native `scatter { dag }` placement — child state is metadata-seeded by a `record-init` node — replacing the hand-dispatch nodes (`recordDispatch`/`draftDispatch` removed).
- **Bounded-memory streaming (no full-corpus OOM).** Per-record results fold through a native compactable `GatherStrategy` (`squashage:record-fold`, `retainsRecordsForFinalize=false`) into bounded run-state counts/samples, replacing the unbounded `services.recordSummaries` sink + `record-summary-collect` node. In `output.mode: 'stream'`, ABox quads and PROV-O provenance stream directly to disk (record writer + prov sidecar) instead of accumulating in `services.dataset`. A `Serializer.openStream` listener-orphan that retained every back-pressured quad batch is fixed. The full 13,653-record aonprd corpus now projects at default heap with ~720 MB peak RSS (previously OOM'd near 4.7 GB).
