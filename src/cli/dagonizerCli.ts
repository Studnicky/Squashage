...
export function buildDagonizerCli(): Command {
  ...
  program
    ...
    .action(async (opts: BuildOptionsInterface): Promise<void> => {
      ...
      process.stdout.write(
        ...
        `lifecycle: ${finalState.lifecycle.kind}\n`,
      );
  ...
  program
    ...
    .action(async (opts: BootstrapOptionsInterface): Promise<void> => {
      ...
      if (opts.skipInduce === true) {
        ...
        if (refineState.lifecycle.kind !== 'completed') {
          process.stderr.write(
            `[refine] phase failed (lifecycle: ${refineState.lifecycle.kind})\n`,
          );
          process.exitCode = 1;
          return;
        }
      ...
      // Note: run.services.recordSummaries also holds the per-record data, but
      // executeBootstrap copies it into finalState.results for convenience.
    });
...
if (isMain) {
  buildDagonizerCli().parseAsync(process.argv).catch((err: unknown) => {
