...
function extractNodeShapes(quads: ReadonlyArray<Quad>): ReadonlyArray<NodeShapeDescriptorInterface> {
...
function resolveClassName(
  ...
  if (shape.targetClassIri !== undefined) return lastSegment(shape.targetClassIri);
  if (ontology !== null) {
  ...
  const derived = lastSegment(shape.shapeIri);
...
async function buildValidationPair(
  shape:   NodeShapeDescriptorInterface,
...
export class ShaclShapeClassifierNode
  implements NodeInterface<SquashageRecordState, Output, SquashageServices> {

  readonly name    = 'classify:shacl-shape';
  ...
  async execute(
    ...
    if (this.#ontologyMode && ontology === null) return { output: 'no-match' };

    const allShapeQuads: ReadonlyArray<Quad> = this.#fileShapes !== null
      ? this.#fileShapes
      : await ontology!.shacl();
    if (allShapeQuads.length === 0) return { output: 'no-match' };

    const shapes = extractNodeShapes(allShapeQuads);
    if (shapes.length === 0) return { output: 'no-match' };

    const matches: Array<{ className: string; targetClassIri: string }> = [];
    for (const shape of shapes) {
      const className = resolveClassName(shape, ontology);
      if (className === undefined) continue;

      const { shapesDataset, dataDataset } = await buildValidationPair(shape, state.input, context.services);
      let conforms = false;
      try {
